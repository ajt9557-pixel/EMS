...
const Leavelist = () => {
  ...
  useEffect(() => {
    const fetchLeaves = async () => {
      try {
        const response = await axios.get(`${API_URL}/api/leave/my-leaves`, authHeader)
      ...
      <div className="relative overflow-hidden rounded-2xl p-6 sm:p-8 bg-gradient-to-r from-blue-500 to-indigo-600 text-white">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-blue-100">My Leaves</p>
              <h2 className="text-2xl font-bold mt-1">
                Welcome, {user ? user.name.split(' ')[0] : 'Employee'}
              </h2>
              <p className="text-sm text-blue-100 mt-1">Track and manage your leave requests</p>
            </div>
          </div>
          <Link
          ...
          <Datatable
            ...
            noDataComponent={
              <div className="py-20 text-center">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Click "Apply Leave" to submit your first request</p>
              </div>
            }
